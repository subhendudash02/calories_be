...
def delete_user(username: str, check: bool = Depends(is_logged_in), role: int = Depends(check_role)):
    if not check:
    ...
    if role == 0 and username:
        raise HTTPException(status_code=400, detail="User can't delete their data. Contact the admin.")
